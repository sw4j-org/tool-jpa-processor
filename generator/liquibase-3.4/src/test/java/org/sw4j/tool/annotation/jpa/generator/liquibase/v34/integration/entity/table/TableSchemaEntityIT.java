 ...
 */
package org.sw4j.tool.annotation.jpa.generator.liquibase.v34.integration.entity.table;
 ...
 * @author Uwe Plonus
 */
public class TableSchemaEntityIT extends ITSuperclass {

    @Test
    public void testChangeSetElement() throws XPathExpressionException {
        Node changeSet = getNode("/databaseChangeLog" +
                "/changeSet[starts-with(@id, 'createTable_TableSchemaEntity')]");
        ...
        Assert.assertTrue(getAttribute("id", changeSet).getNodeValue().startsWith("createTable_TableSchemaEntity"),
                "Expected the id attribute to start with \"createTable_TableSchemaEntity\"");
    ...
    public void testCreateTableElement() throws XPathExpressionException {
        Node createTable = getNode("/databaseChangeLog" +
                "/changeSet[starts-with(@id, 'createTable_TableSchemaEntity')]" +
                "/createTable[@tableName='TableSchemaEntity']");
        ...
        Assert.assertEquals(getAttribute("tableName", createTable).getNodeValue(), "TableSchemaEntity",
                "Expected the tableName attribute to be \"TableSchemaEntity\"");
    }

    @Test
    public void testTableCatalogElement() throws XPathExpressionException {
        Node createTable = getNode("/databaseChangeLog" +
                "/changeSet[starts-with(@id, 'createTable_TableSchemaEntity')]" +
                "/createTable[@tableName='TableSchemaEntity']");

        Assert.assertNotNull(createTable, "Expected a createTable to be created.");
        Assert.assertNotNull(getAttribute("schemaName", createTable),
                "Expected the createTable to have a schemaName attribute.");
        Assert.assertEquals(getAttribute("schemaName", createTable).getNodeValue(), "SCHEMA",
                "Expected the schemaName attribute to be \"SCHEMA\"");
    }
